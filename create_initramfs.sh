...
then
    RFSBUILD=$HOME/$rfsbuilddir
        RFSOPT=$HOME/$rfsoptdir
        mkdir -p $RFSBUILD
        if [ -f "/opt/zipimages/busybox-${bboxversion}.tar.bz2" ]
        then
            echo "busybox-${bboxversion}.tar.bz2 is found on /opt/zipimages. Using it to Local Repo"
                cp /opt/zipimages/busybox-${bboxversion}.tar.bz2 $RFSBUILD
        else
            echo "busybox-${bboxversion}.tar.bz2 is not found on /opt/zipimages. Downloading it from Cloud Repo"
...
echo "\nBoot took $(cut -d' ' -f /proc/uptime) seconds\n"
exec /bin/sh
EOF
